...
public class GugudanFrame extends JFrame {
	...
	// 필요한 경우 이 프레임의 하위 컴포넌트에서 현재 컴포넌트의 모든것을 알 수 있도록 데이터를 전달해주는 편이 편하다
	JButton prevBtn = new PrevButton(this);
	JButton nextBtn = new NextButton(this);
	
	// 이벤트 모음 (인스턴스 변수로 만듦)
	ActionListener guguActionListener;
	...
	public GugudanFrame(int size) {
		...
		guguActionListener = new GugudanButtonActionListener(this);
		...
		// 이벤트 추가 ( 모든 컴포넌트들을 초기화 한 이후에 붙이는 것이 좋다) -> 할수있으면 묶어놓기
		prevBtn.addActionListener(guguActionListener);
		nextBtn.addActionListener(guguActionListener);
