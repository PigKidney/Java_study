...
public class E01_Exception {
	/*
	 	...
	 	- 프로그램 실행 도중 문제 생겨 발생하는 에러들을 예외라고 부른다
	 	...
	 	- try문 내부에서 예외가 발생하지 않으면 평소문대로 정상 작동한다
	 	- try문 내부에서 예외가 발생하면 즉시 try문을 중단하고
	 	  발생한 예외에 해당하는 catch문으로 이동한다
	 	- 예외 처리의 기본 동작은 예외 발생 당시 상황을 콘솔에 출력하고
	 	  프로그램을 강제 종료하는 것이다 (System.exit(), return)  
