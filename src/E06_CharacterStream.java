...
public class E06_CharacterStream {
	...
	/*
	 	...
	 	- char 담위로 데이터를 입출력하는 클래스
	 	- 프로그래머는 데이터를 char단위로 전달하면 클래스에서 알아서 byte단위로 분해하여 스트림으로 전송한다 (Writer)
		- 스트림으로 읽은 데이터를 프로그래머에게 전달할 때는 알아서 char으로 합쳐서 전달한다 (Reader)
	*/
	public static void main(String[] args) {
		
		// 데이터를 문자 단위로 다룬다는 것 -> Charset을 사용한다는 것
		
		// 똑같은 2진법 데이터라도 문자 해석 방식(charset)에 따라 다른 문자가 될 수 있다
		
		// Charset 종류 : UTF-8, UTF-16, EUC-KR, MS949, ....
		
		// FileWriter : 문자 단위로 파일에 쓸 수 있는 output
		try {
			FileWriter out = new FileWriter("myfiles/test2.txt", Charset.forName("EUC-KR"));
...
// -----------------------------------------------------------------------------------------------------------			
			...
			// 읽은 글자의 길이를 반환하고 파일의 끝에 도달하먄 -1을 리턴한다.
