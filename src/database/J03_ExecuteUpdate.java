...
public class J03_ExecuteUpdate {
	 ...
	 * # pstmt.executeUpdate()
	 * 
	 * - INSERT, UPDATE, DELERE문에 사용한다. 결과로 int값이 나온다. - int 값으로는 반영된 행의 개수가 리턴된다
	 */

	public static void main(String[] args) {
		try (Connection conn = JdbcConnection.getConnection();) {
			// 기본키를 넣을 때는 자바쪽에서 시퀀스를 불러 사용한다
			String sql1 = "INSERT INTO fruits(fruit_id, fruit_name, fruit_price) VALUES(fruit_id_seq.nextval,?,?)";
			try (PreparedStatement pstmt = conn.prepareStatement(sql1)) {
				pstmt.setString(1, "Peach");
				pstmt.setInt(2, 1500);

				// INSERT, UPDATE, DELETE는 executeUpdate()로 싱행해야한다
