...
public class OmokBoard {
	...
	public boolean winCheck(String coordStr, boolean isBlack) {
		...
		// 3-2) 현재 위치에서부터 오른쪽 아래쪽으로 가면서 개수 세기
	...
	private int[] getCoordArray(String coordStr) {
		...
		return new int[] { coordStr.length() == 2 ? coordStr.charAt(1) - '0' - 1 : 10 + (coordStr.charAt(2) - '0') - 1,
				coordStr.charAt(0) - 'A' };
	...
	public boolean put(String coordStr, boolean isBlack) {
		...
		if (row >= MAX_HEIGHT || col >= MAX_WIDTH) {
			System.out.println("[Error] 좌표값이 이상합니다...");
			return false;
